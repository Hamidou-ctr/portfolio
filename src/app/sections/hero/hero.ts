...
@Component({
  ...
  template: `
    ...
    >
      <div class="relative mx-auto grid w-full max-w-6xl gap-10 px-6 py-16 md:grid-cols-2 md:items-center lg:px-10">
        ...
        <div class="order-2 text-center md:order-none md:text-left">
          <p class="mb-2 text-sm font-medium uppercase tracking-widest text-white/60">{{ t().hero.iAm }}</p>
          <h1 class="font-heading text-4xl font-bold sm:text-5xl">{{ profile.name }}</h1>
          <p class="mt-1 font-heading text-xl font-semibold text-violet-400 sm:text-2xl">{{ t().hero.role }}</p>
          ...
          <div class="mt-10 flex items-center justify-center gap-4 border-t border-white/10 pt-6 md:justify-start">
            <a [href]="profile.github" target="_blank" rel="noopener" aria-label="GitHub">
              <img ngSrc="assets/img/github_button.png" width="30" height="30" alt="" class="h-6 w-6" />
            </a>
            <a [href]="profile.linkedin" target="_blank" rel="noopener" aria-label="LinkedIn">
              <img ngSrc="assets/img/linkedin_button.png" width="30" height="31" alt="" class="h-6 w-6" />
            </a>
            <a [href]="'mailto:' + profile.email" aria-label="Email" class="flex items-center gap-2">
              <img ngSrc="assets/img/email_button.png" width="30" height="31" alt="" class="h-6 w-6" />
      ...
      >
        {{ t().hero.scrollDown }}
        <img ngSrc="assets/img/scroll_down.png" width="16" height="16" alt="" class="h-3 w-3 -rotate-90" />
      </a>
    </section>
  `,
})
