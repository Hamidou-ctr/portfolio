...
@Component({
  ...
  template: `
    <section id="portfolio" class="relative scroll-mt-20 overflow-hidden bg-white py-20 md:py-28">
      <div class="pointer-events-none absolute -right-32 top-0 z-0 h-[600px] w-[600px] opacity-70" aria-hidden="true">
        <img ngSrc="assets/img/purple_shadow_portfolio.png" fill class="object-contain" alt="" />
      </div>

      <div class="relative mx-auto max-w-6xl px-6 text-center lg:px-10">
        <h2 class="font-heading text-3xl font-bold text-ink sm:text-4xl">{{ t().portfolio.title }}</h2>
      ...
      <ul class="relative mx-auto mt-16 flex max-w-6xl flex-col gap-20 px-6 lg:px-10">
        @for (project of projects; track project.name) {
          <li class="grid items-center gap-10 md:grid-cols-2">
            <div class="relative aspect-square overflow-hidden rounded-2xl bg-slate-50 shadow-lg shadow-slate-200/60">
              @if (project.previewSrc && project.previewSrc.endsWith('.svg')) {
                <img [src]="project.previewSrc" [alt]="project.name" class="h-full w-full object-contain p-8" />
              } @else if (project.previewSrc) {
                <img [ngSrc]="project.previewSrc" fill class="object-contain p-8" [alt]="project.name" />
              } @else {
